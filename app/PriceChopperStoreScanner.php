...
class PriceChopperStoreScanner extends StoreScanner
{
    protected $baseUri = 'https://shop.pricechopper.com/api/v2/';
    ...
    private function changeStore(Store $store) {
        ...
        if ($user->store->id != $store->identifier) {
            throw new RuntimeException('PriceChopper: Expected store id to be ' . $store->identifier . ' but got ' . $user->store->id);
        }
    }
